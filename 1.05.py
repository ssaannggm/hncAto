...
file_path_frame = tk.Frame(
    root, highlightbackground="black", highlightthickness=1, padx=10, pady=10
)
...
notice_labe2 = tk.Label(
    file_path_frame,
    text="파일 선택 눌러서 편집할 파일을 연결 / 연결 안되면 다 끄고 다시 해주세요...(후졌음)",
    font=("Arial", 10),
    anchor="w",
    fg="red",
)
...
file_path_label = tk.Label(
    file_path_frame, text="파일 경로: 없음", font=("Arial", 10), anchor="e"
)
...
def bind_button_to_key(button, key, modifier=None):
    ...
    def key_action(event=None):
        ...
        root.after(
            100, lambda: button.config(relief="raised")
        )  # 100ms 후 원래 상태 복구
...
##########################################
# region '표 기본설정' 프레임
frame_init_macro = tk.Frame(
    tab1, highlightbackground="black", highlightthickness=2, padx=10, pady=10
)
frame_init_macro.grid(row=0, column=1, padx=5, pady=5, sticky="ew")

init_macro_label = tk.Label(
    frame_init_macro, text="표 기본설정", font=("Arial", 12, "bold")
)  # 기본 정렬 / 전체, 2단 채우기, 입실론, 그림 캡션
...
check5 = tk.Checkbutton(
    frame_init_macro, text="셀단위로나눔,제목반복(2)", variable=var5
)
...
index_spinbox = tk.Spinbox(
    frame_modi_macro, from_=-100, to=100, increment=0.1, textvariable=align_ep, width=3
)
...
dropdown8 = ttk.Combobox(
    frame_cell_macro,
    values=["표주 없음(냅둠)", "0.12(아래투명/위는0.12)", "0.4(아래투명/위0.4)"],
)
...
btn_선없음 = tk.Button(
    frame_cell_macro, text="  All선없음 [Y]", command=표라인_전체투명
)
...
##########################################
# region '그림 이동' 프레임
frame_pic_movement = tk.Frame(
    tab2, highlightbackground="black", highlightthickness=2, padx=10, pady=10
)
frame_pic_movement.grid(row=0, column=0, pady=5, sticky="ew")

position_pic = tk.Label(
    frame_pic_movement, text="그림 이동", font=("Arial", 12, "bold")
)
...
index_spinbox = tk.Spinbox(
    frame_pic_movement, from_=0, to=0, textvariable=current_pic_index, width=5
)
index_spinbox.grid(row=1, column=1, padx=5, pady=5)
...
btn_begin2 = tk.Button(
    frame_pic_movement, text="처음으로 []", command=처음으로_그림
)  ####그림ㅊ퍼음으로
...
##########################################
# region '그림 기본 설정'프레임
frame_pic_init_macro = tk.Frame(
    tab2, highlightbackground="black", highlightthickness=2, padx=10, pady=10
)
frame_pic_init_macro.grid(row=0, column=1, padx=5, pady=5, sticky="nsew")

init_macro_label = tk.Label(
    frame_pic_init_macro, text="그림 기본설정", font=("Arial", 12, "bold")
)  # 기본 정렬 / 전체, 2단 채우기, 입실론, 그림 캡션
init_macro_label.grid(row=0, column=0, columnspan=2, padx=5, pady=5)
...
pic_check1 = tk.Checkbutton(
    frame_pic_init_macro, text="글자해제,자리차지", variable=pic_var1
)
pic_check2 = tk.Checkbutton(
    frame_pic_init_macro, text="그림캡션달기", variable=pic_var2
)
pic_check3 = tk.Checkbutton(frame_pic_init_macro, text="밖여백 0mm", variable=pic_var3)
pic_check4 = tk.Checkbutton(
    frame_pic_init_macro, text="안여백 0mm, 외곽선없음", variable=pic_var4
)
...
pic_button3 = tk.Button(
    pic_pos_frame, text="2단 오른 상[]", command=그림_2단_오른_상
)  ##
...
position_para = tk.Label(
    para_frame, text="글머리 패턴-스타일", font=("Arial", 12, "bold")
)
...
root.geometry(
    f"{max(200, 15+ notebook.winfo_reqwidth())}x{max(300, notebook.winfo_reqheight() + 180)}"
)
